 ...
 * @author xiafan
 */
public class DynamicDiffProcessor extends DifficultyProcessor {
    @Override
    public void process(Map<String, Object> searchConditions, SearchContext context) {
        backwardProcess(searchConditions, context);

        if (searchConditions.get("isDifficulty") != null) {
            if (searchConditions.get("regYear") != null
                    && !searchConditions.get("regYear").toString().equals(SearchSQLService.STRING_FOR_WHOLE_DATA)) {
                context.stringEqual(Table.DIFFICULTY, Column.YEAR, searchConditions.remove("regYear").toString());
            }

            if (searchConditions.get("term") != null
                    && !searchConditions.get("term").toString().equals(SearchSQLService.STRING_FOR_WHOLE_DATA)) {
                context.numericEqual(Table.DIFFICULTY, Column.YEAR, searchConditions.remove("term").toString());
            }
            ...
            if (!isDiff.equals(SearchSQLService.STRING_FOR_WHOLE_DATA)) {
                ...
                context.leftJoin(Table.DIFFICULTY, Column.STUDENT, Table.STUDENT, Column.ID);
                context.setMapper(new DifficultyMapper(context.getMapper()));
                context.addColumn(Table.DIFFICULTY, "*");
            }
        }
